...
4. אסור להמציא. אל תמציא מחשבות או כוונות של השחקן ("רצית לתקוף את המלך") אלא אם הוא אמר זאת בעצמו - לכל היותר אפשר לכתוב "אם הרעיון שלך היה X, הבעיה היא...". אל תמציא טקטיקות, איומים או מהלכים שלא באמת קיימים בעמדה שסופקה - אם אתה טוען שיש מזלג, ריתוק, מהלך שמאיים על כלי, או שמהלך מסוים אפשרי, זה חייב להיות אמיתי ומבוסס על הנתונים שסופקו, לא ניחוש.
5. אל תמציא בעיה כשאין אחת. אם המהלך היה טוב, תגיד שהוא טוב ותסביר קונקרטית למה - אל תחפש בכוח משהו לבקר. ואם אין לך מספיק מידע כדי לזהות סיבה אמיתית וברורה, עדיף להודות בכך בקצרה מאשר להמציא הסבר שנשמע משכנע אך אינו מבוסס.
6. אל תשתמש בשפה דרמטית או מביישת סביב טעויות חמורות (למשל "איזה בלנדר מטורף!") - גם טעות חמורה מוסברת בטון תומך: קודם לומר בבירור שהייתה כאן טעות משמעותית, ומיד להסביר בדיוק למה ומה קורה בעקבותיה.`;
...
export function levelProfile(playerElo) {
  ...
  // The exact rating (not just a bucketed range) is always surfaced to the coach
  // when known - a 650 and a 950 are both "beginner" tier, but 650 still needs
  // noticeably more hand-holding than 950, and the coach should be able to feel
  // that difference rather than treating the whole tier as one flat level.
  const eloNote = playerElo != null ? `${playerElo} בדירוג אלו בדיוק` : null;
  if (playerElo != null && playerElo <= 500) {
    return {
      simple: true,
      persona,
      audienceLabel: `שחקן מתחיל לגמרי (${eloNote}) שרק התחיל ללמוד שחמט`,
      promptRules:
  ...
  if (playerElo != null && playerElo <= 900) {
    return {
      simple: true,
      persona,
      audienceLabel: `שחקן מתחיל (${eloNote})`,
      promptRules:
  ...
  if (playerElo != null && playerElo <= 1400) {
    return {
      simple: false,
      persona,
      audienceLabel: `שחקן בדרגת ביניים (${eloNote})`,
      promptRules:
  ...
  return {
    simple: false,
    persona,
    audienceLabel: eloNote ? `שחקן מנוסה (${eloNote})` : 'שחקן מתקדם, או ללא רמת קושי מוגדרת',
...
export function buildMovePrompt({
  ...
  const continuationText = continuationSans?.length
    ? `רצף ההמשך שהמנוע רואה כענישה הצפויה של היריב (מהלך אחר מהלך, החל מתגובת היריב למהלך): ${continuationSans.join(', ')}. אל תשטח את כל הרצף הזה מילה במילה למשתמש - קח ממנו את הרעיון המרכזי (איזה כלי נופל, איזו מתקפה נפתחת, מה נהרס) ותאר אותו כסיפור קצר.`
    : null;

  const mateText =
    mateAfter != null
      ? `שים לב: המנוע מזהה כאן מט-בכפייה (${mateAfter > 0 ? `מט תוך ${Math.abs(mateAfter)} מהלכים לטובת ${colorLabel}` : `מט תוך ${Math.abs(mateAfter)} מהלכים נגד ${colorLabel}`}) - זה שונה לגמרי מירידת הערכה רגילה. אם זה איום נגד השחקן, אתה חייב להסביר את האיום הממשי בפועל (למשל "עכשיו יש ליריב מט תוך שני מהלכים"), לא רק לדווח על מספר.`
      : mateBefore != null
        ? `שים לב: לפני המהלך הזה כבר היה מט-בכפייה על הלוח (${mateBefore}) - קח בחשבון שהעמדה כבר הייתה קריטית.`
        : null;

  const toneInstruction = isBad
    ? 'המהלך הזה היה טעות משמעותית. תגיד בבירור שהייתה כאן טעות, ומיד תסביר בדיוק מה השתבש ולאן זה מוביל - בטון תומך, בלי דרמטיות ובלי לבייש.'
    : isMinor
      ? 'המהלך הזה לא היה מדויק לגמרי, אך לא חמור. הסבר בעדינות מה אפשר היה לשפר - אל תגזים בחומרה.'
      : 'המהלך הזה היה טוב או מיטבי. אל תחפש בכוח בעיה שאין - חזק את השחקן בחיוב והסבר קונקרטית מה עשה נכון ולמה זה עובד בעמדה הזו.';
...
נתוני המהלך (מידע פנימי בשבילך בלבד - אל תצטט מספרי הערכה גולמיים אם ההנחיה למעלה אוסרת זאת, ואל תצטט "עומק" או "confidence" למשתמש בכלל):
- מספר מהלך: ${moveNumber} (${colorLabel})
- מצב הלוח לפני המהלך (FEN): ${fenBefore}
${opponentLastMoveSan ? `- המהלך האחרון של היריב, לפני המהלך הזה: ${opponentLastMoveSan}` : ''}
- המהלך ששוחק בפועל: ${san}
${fenAfter ? `- מצב הלוח אחרי המהלך (FEN): ${fenAfter}` : ''}
- סיווג המהלך על ידי מנוע השחמט: ${classificationLabel}
${bestMoveSan && bestMoveSan !== san ? `- המהלך המומלץ במקום (לפי Stockfish - זהו מקור האמת היחיד למהלך הטוב ביותר, אל תציע מהלך "טוב יותר" משלך): ${bestMoveSan}` : '- זהו היה המהלך הטוב ביותר (או קרוב לכך) בעמדה לפי המנוע'}
- הערכת העמדה לפני המהלך: ${evalBeforeStr}
- הערכת העמדה אחרי המהלך: ${evalAfterStr}
${mateText ? `- ${mateText}` : ''}
${continuationText ? `- ${continuationText}` : ''}

לפני שאתה כותב את התשובה, בצע בעצמך את ההשוואה הבאה בין fenBefore ל-fenAfter (אל תציג את ההשוואה עצמה למשתמש, רק את המסקנות ממנה): מה בדיוק זז, מה נחשף או נסגר בעקבות המהלך, איזה כלי (אם בכלל) הפך להיות מותקף או הפסיק להיות מוגן, והאם המלך של מישהו נחשף. הסיבה שאתה נותן לטעות (או לשבח) חייבת לנבוע ישירות מהעובדה הזו שמצאת בעמדה - לא מהיגיון כללי כמו "זה תמיד רע לזוז מוקדם מדי". ודא שאתה יכול לענות בעצמך על כל אחת מהשאלות הבאות על סמך הנתונים שסופקו, לפני שאתה כותב: (1) מה השחקן עשה, (2) מה היריב יכול לעשות בעקבות זה, (3) למה זה בעייתי או טוב, (4) מה היה עדיף (אם רלוונטי), (5) מה השחקן צריך לזכור להבא. אם אינך יכול לענות על אחת מהן במדויק מהנתונים שסופקו - אל תמציא תשובה, תישאר כללי וזהיר יותר באותו חלק בלבד.
