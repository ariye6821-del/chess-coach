  ...
  const wireConnection = (conn, colorForMe) => {
    ...
    const timeoutId = setTimeout(() => {
      if (connRef.current === conn) {
        ...
        setMyColor(null);
        setErrorMessage(
          'לא הצלחנו להתחבר לחבר - ייתכן שהרשת שלכם (רשת סלולרית, או אפליקציית סינון/הגנה כמו רימון) חוסמת חיבור ישיר בין מכשירים. נסו רשת Wi-Fi אחרת, או נסו שוב. אם יש לכם אפליקציית סינון תוכן, ייתכן שהיא חוסמת את הטכנולוגיה הזו ולא ניתן לעקוף זאת מתוך האפליקציה.'
        );
      }
    }, 20000);
    conn.on('open', () => {
      ...
      <div dir="rtl" className="mx-auto max-w-md space-y-5 text-center">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            שימו לב: המצב הזה דורש רשת שמאפשרת חיבור ישיר בין מכשירים. חלק מרשתות סלולריות ואפליקציות סינון/הגנה (כמו
            רימון) חוסמות חיבור מסוג זה.
          </p>
        </div>
        <button
          onClick={createRoom}
